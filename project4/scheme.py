...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        ...
        if expressions.rest is nil:
            tail=True
        s=scheme_eval(expressions.first, env,tail)
        if expressions.rest is nil:
            return s
...
def do_and_form(expressions, env):
    ...
    if expressions.rest==nil:
        ...
        de=scheme_eval(expressions.first,env)
        if scheme_truep(de):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbol=Pair(bindings.first.first,symbol)
        check_formals(symbol)
        val=Pair(v,val)
        bindings=bindings.rest
    return env.make_child_frame(symbol,val)
    # END PROBLEM 14
